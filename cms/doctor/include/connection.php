...
function displayData_report($arr,$query,$pgEdit,$pgDelete,$folder)
{
	$cnt=1;
	$cn=getCon();
	$rs=mysqli_query($cn,$query) or die(mysqli_error());
	...
	while($row=mysqli_fetch_array($rs))
	{
			...
			$table=$table."<td>
			<a href=\"$pgEdit?id=$row[0]\"><button class=\"btn btn-sm btn-danger\"><i class=\"glyphicon glyphicon-pencil\"></i>Report View</button></a> 
...
function displayData_appoitmentd($arr,$query,$pgEdit,$pgDelete,$folder)
{
	$cnt=1;
	$cn=getCon();
	$rs=mysqli_query($cn,$query) or die(mysqli_error());
...
function displayData_appoitmente($arr,$query,$pgEdit,$pgDelete,$folder)
{
	$cnt=1;
	$cn=getCon();
	$rs=mysqli_query($cn,$query) or die(mysqli_error());
	...
	$table=$table."<th width=\"150px\"></th></tr></thead><tbody>";	
	while($row=mysqli_fetch_array($rs))
	{
			$table=$table."<tr class=\"odd gradeX\"><td> $cnt</td>";
			for($i=1;$i<= sizeof($arr);$i++)
...
function displayData_appoitmentcancel($arr,$query,$pgEdit,$pgDelete,$folder)
{
	$cnt=1;
	$cn=getCon();
	$rs=mysqli_query($cn,$query) or die(mysqli_error());
	...
	while($row=mysqli_fetch_array($rs))
	{
			...
			$table=$table."<td>
			<a href=\"$pgEdit?id=$row[0]\"><button class=\"btn btn-sm btn-primary\"><i class=\"glyphicon glyphicon-pencil\"></i></button></a> 
			<a href=\"$pgDelete?id=$row[0]\" onclick=\"if(!confirm('Are you sure that you want to cancel?'))return false\"><button class=\"btn btn-sm btn-danger\"><i class=\"glyphicon glyphicon-trash\"></i> ReApoint</button></a>
			</td></tr>";
	$cnt++;
	}
	$table=$table."</tbody></table>";
